...
Sphere.getVertex = function (uu, vv) {
    const vd = {};
    const u = uu * Math.PI;//vertical axis
    const v = vv * 2 * Math.PI;//horizontal axis
    
    //fixed version correctly converts from spherical to cartesian coordinates
    vd.position = vec4(
        //Math.cos(u) * Math.sin(v), //original version
        Math.sin(u) * Math.cos(v),//changed
    ...
    //vd.texCoord = vec2(uu, vv);//original
    vd.texCoord=vec2(1.0-vv, uu); //rotates texture 90 deg, map vv bottom to top, uu left to right. then flip vv to match texture orientation
...
Sphere.draw = function (cullFace=0) {
...
// Tetrahedron//
Tetrahedron = {};
Tetrahedron.numVertices = 12; // 4 triangles * 3 vertices each
const sqrt_3 = Math.sqrt(3);
const sqrt_3_3 = sqrt_3 / 3;
Tetrahedron.pointsArray = [];
Tetrahedron.normalsArray = [];
Tetrahedron.colorsArray = [];
Tetrahedron.texCoordsArray = [];

Tetrahedron.vertices = [
    vec4(0, 0, 1, 1),  // V_top
    vec4(0,-Math.sqrt(6)/3, -1/3, 1),  // V_1
    vec4(Math.sqrt(2)/2,Math.sqrt(6)/6,  -1/3, 1),  // V_2
    vec4( -Math.sqrt(2)/2,Math.sqrt(6)/6, -1/3, 1)   // V_3
];

// Indices of the vertices per triangle face
Tetrahedron.indices = [
    0, 1, 2,
    0, 2, 3,
    0, 3, 1,
...
Tetrahedron.init = function (program) {
    for (let i = 0; i < this.indices.length; i += 3) {
        ...
        this.normalsArray.push(normal, normal, normal);
